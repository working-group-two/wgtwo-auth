...
public class WgtwoAuth implements Closeable {
  ...
  private WgtwoAuth(
    ...
    ServiceBuilder builder =
        new ServiceBuilder(clientId).apiSecret(clientSecret).callback(callback);
    if (debug) {
  ...
  public class AuthorizationCode {
    ...
    public Token refreshToken(@NotNull String refreshToken) {
      Instant now = clock.instant();
      try {
        OpenIdOAuth2AccessToken response =
            (OpenIdOAuth2AccessToken) service.refreshAccessToken(refreshToken);
        return createToken(now, response);
      } catch (OAuth2AccessTokenErrorResponse e) {
        throw new AccessTokenException(e.getError().getErrorString(), e.getErrorDescription());
      } catch (IOException | ExecutionException | InterruptedException e) {
    ...
    private Token createToken(Instant now, OpenIdOAuth2AccessToken token) {
      ...
      return new Token(
          token.getAccessToken(), token.getRefreshToken(), expiry, token.getScope(), metadata);
    }
  }
}
