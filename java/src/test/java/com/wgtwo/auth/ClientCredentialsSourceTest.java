...
class ClientCredentialsSourceTest {
    ...
    private static final String SCOPE_SUBSCRIPTION_HANDSET_DETAILS_READ = "subscription.handset_details:read";
    ...
    public void shouldCacheUsingClientCredentialsSource() {
        mockServer.when(
                ...
                response()
                        .withStatusCode(200)
                        .withBody("{\"access_token\":\"my-access-token\",\"expires_in\":3599,\"scope\":\""+SCOPE_SUBSCRIPTION_HANDSET_DETAILS_READ+"\",\"token_type\":\"bearer\"}")
        ...
        ClientCredentialSource clientCredentialSource = wgtwoAuth.clientCredentials.newTokenSource(SCOPE_SUBSCRIPTION_HANDSET_DETAILS_READ);
    ...
    private WgtwoAuth createWgTwoAuthWithFakeClock() {
        return WgtwoAuth.builder("86b76cb5-caf3-4c1c-bdfe-8b3454f580b8", "RzvcbQDfrkOb5ElvPuxA49oA8odBZfvj5MK1r2AdFuV99EGvL4aJvARUg637p3QqqgrU6gyG")
                .oauthServer("http://127.0.0.1:" + mockServer.getLocalPort())
                .clock(clock)
                .build();
    }
    ...
    static Stream<Arguments> providerOfWithinRefreshIntervalOrLater() {
        return Stream.of(
                ...
                Arguments.of(named("'within' life time of an access token but after expiry", Duration.ofMinutes(42))),
                Arguments.of(named("long after life time of both access token and expiry", Duration.ofDays(1)))
        );
    }
    ...
    private void createToken(String accessToken) {
        mockServer.when(
                ...
                response()
                        .withStatusCode(200)
                        .withBody("{\"access_token\":\"" + accessToken + "\",\"expires_in\":3599,\"scope\":\""+SCOPE_SUBSCRIPTION_HANDSET_DETAILS_READ+"\",\"token_type\":\"bearer\"}")
                        .withDelay(Delay.milliseconds(250))
        );
    }
