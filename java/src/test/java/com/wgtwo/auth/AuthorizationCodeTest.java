...
class AuthorizationCodeTest {
  ...
  public void shouldCreateAuthorizationCode() {
    WgtwoAuth wgtwoAuth =
        WgtwoAuth.builder("clientId", "clientSecret")
    ...
    String authorizationUrl =
        wgtwoAuth.authorizationCode.createAuthorizationUrl(scope, nonce, state, Prompt.DEFAULT);
  ...
  public void shouldExchangeAuthorizationCodeForAccessTokenWithoutOpenIdToken() {
    ...
    WgtwoAuth wgtwoAuth =
        WgtwoAuth.builder("my-client-id", "my-client-secret")
            .callbackUri("https://localhost:30000/oauth/callback")
            .oauthServer("http://127.0.0.1:" + mockServer.getLocalPort())
            .build();

    Token token =
        wgtwoAuth.authorizationCode.fetchToken(
            "7CjFbYFYpr9IJ0CUJTlKZvt3kGr1k2fY47zWm8_Jn2o.hw8mJQKNVXU2jRprG-2bv4ppw8DVXzo4COQbXgKTbN0");
    assertThat(token.getMetadata()).isNull();
    assertThat(token.getAccessToken())
        .isEqualTo(
            "ih_iwZar30-sjSJiJkBRsNePNZ_MGjhmhgAwMg6tLr0.YzzYG3UIUOb9W8XFZkUQ1S0OuIJE5mvmSGsO1cBx_RE");
    ...
    mockServer
        .when(
            request()
                ...
                .withContentType(MediaType.APPLICATION_FORM_URLENCODED)
                .withBody(
                    "code=7CjFbYFYpr9IJ0CUJTlKZvt3kGr1k2fY47zWm8_Jn2o.hw8mJQKNVXU2jRprG-2bv4ppw8DVXzo4COQbXgKTbN0&redirect_uri=https%3A%2F%2Flocalhost%3A30000%2Foauth%2Fcallback&grant_type=authorization_code"))
        .respond(
            response()
                .withStatusCode(200)
